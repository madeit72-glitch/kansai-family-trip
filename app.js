...
const setTimePicker=(prefix,time='10:00')=>{const [rawHour='10',minute='00']=time.split(':');const hour=Number(rawHour);$(`#${prefix}Period`).value=hour>=12?'PM':'AM';$(`#${prefix}Hour`).value=String(hour%12||12);$(`#${prefix}Minute`).value=String(Number(minute)||0)};
const parseSchedule=x=>{const meta=x[3]||{},parts=String(x[2]||'').replace(/^~/,'').split(' · '),end=meta.end||parts.shift()||x[0],place=meta.place??parts.shift()??'',detail=meta.detail??parts.join(' · ');return{start:x[0],title:x[1],end,place,detail}};
...
function render(){
  ...
  $('#scheduleList').innerHTML=schedules.length?schedules.map((x,i)=>`<article class="timeline-item" data-edit-schedule="${i}" tabindex="0" role="button" aria-label="${escapeHtml(x[1])} 일정 수정"><time class="timeline-time">${escapeHtml(x[0])}</time><i class="timeline-dot"></i><div class="timeline-card"><span class="edit-hint">눌러서 수정</span><h3>${escapeHtml(x[1])}</h3><p>${escapeHtml(x[2])}</p></div></article>`).join(''):'<div class="empty">등록된 일정이 없습니다. 첫 일정을 추가해 보세요.</div>';
...
document.addEventListener('click',e=>{
  ...
  const open=e.target.closest('[data-open]'); if(open){const type=open.dataset.open;$('#entryForm').reset();$('#entryType').value=type;$('#entryIndex').value='';$('#dialogTitle').textContent=type==='schedule'?'일정 추가':'예약 추가';$('#timeField').hidden=type!=='schedule';$('#placeField').hidden=type!=='schedule';$('#codeField').hidden=type!=='booking';$('#deleteEntry').hidden=true;setTimePicker('start','10:00');setTimePicker('end','11:00');$('#entryDialog').showModal();return}
  const edit=e.target.closest('[data-edit-schedule]');if(edit){const index=Number(edit.dataset.editSchedule),item=state.schedules[state.selected]?.[index];if(!item)return;const parsed=parseSchedule(item);$('#entryForm').reset();$('#entryType').value='schedule';$('#entryIndex').value=String(index);$('#dialogTitle').textContent='일정 수정';$('#timeField').hidden=false;$('#placeField').hidden=false;$('#codeField').hidden=true;$('#deleteEntry').hidden=false;$('#entryTitle').value=parsed.title;$('#entryPlace').value=parsed.place;$('#entryDetail').value=parsed.detail;setTimePicker('start',parsed.start);setTimePicker('end',parsed.end);$('#entryDialog').showModal();return}
...
document.addEventListener('keydown',e=>{if((e.key==='Enter'||e.key===' ')&&e.target.matches('[data-edit-schedule]')){e.preventDefault();e.target.click()}});
$('#closeDialog').onclick=()=>$('#entryDialog').close();
$('#deleteEntry').onclick=()=>{const index=Number($('#entryIndex').value),item=state.schedules[state.selected]?.[index];if(!item||!confirm('이 일정을 삭제할까요?'))return;window.removeSchedulePoint?.(state.selected,item[1],item[0]);state.schedules[state.selected].splice(index,1);save();render();$('#entryDialog').close();toast('일정을 삭제했습니다')};
$('#entryForm').onsubmit=e=>{e.preventDefault();const type=$('#entryType').value,title=$('#entryTitle').value.trim(),detail=$('#entryDetail').value.trim();if(!title)return;if(type==='schedule'){const start=to24Hour($('#startPeriod').value,$('#startHour').value,$('#startMinute').value),end=to24Hour($('#endPeriod').value,$('#endHour').value,$('#endMinute').value),place=$('#entryPlace').value.trim(),fullDetail=`~${end}${place?` · ${place}`:''}${detail?` · ${detail}`:''}`,index=$('#entryIndex').value===''?null:Number($('#entryIndex').value),old=index===null?null:state.schedules[state.selected]?.[index];if(old){window.removeSchedulePoint?.(state.selected,old[1],old[0]);state.schedules[state.selected][index]=[start,title,fullDetail,{end,place,detail}]}else{(state.schedules[state.selected]??=[]).push([start,title,fullDetail,{end,place,detail}])}state.schedules[state.selected].sort((a,b)=>a[0].localeCompare(b[0]));save();window.addSchedulePoint?.(state.selected,title,place,start,detail);toast(old?'일정을 수정했습니다':place?'일정을 저장하고 지도 위치를 확인합니다':'일정을 저장했습니다')}else{(state.bookings[state.selected]??=[]).push(['🎫',title,detail,$('#entryCode').value||'확인 필요']);save();toast('예약을 저장했습니다')}render();$('#entryDialog').close();};
