...
create or replace function public.join_trip(p_code text) returns uuid language plpgsql security definer set search_path=public,pg_temp as $$
declare v_trip uuid;
begin
  if auth.uid() is null then raise exception 'Login required'; end if;
  select trip_id into v_trip from public.trip_invites where code_hash=encode(digest(upper(trim(p_code)),'sha256'),'hex');
...
begin
  select trip_id into v_trip from public.trip_invites where code_hash=encode(digest('KANSAI26','sha256'),'hex');
  if v_trip is null then
    insert into public.trip_rooms(name) values('우리 가족 간사이 여행') returning id into v_trip;
    insert into public.trip_invites(trip_id,code_hash) values(v_trip,encode(digest('KANSAI26','sha256'),'hex'));
  end if;
end $$;
